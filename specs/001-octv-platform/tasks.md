...
- [X] T066 Cascade collision round 2: scope the components-layer `.danger` text utility to `:not(button)` — it was repainting every solid `button.danger` confirm (delete event/trip/comp, remove member, deactivate student, request deletion) as red-on-red, same layer-order family as T061.
- [X] T067 Systematic defect sweep (post-T066): rendered-contrast + overflow clean across ~60 route/width combos and all roles; all internal links + PDF/ICS endpoints return <400; every action ?error= param verified rendered (ERR maps + fallbacks); console clean; fixed the only findings — aria-labels on the three unlabeled controls (chaperone guardian picker, chaperone name override, meal-note textarea).
