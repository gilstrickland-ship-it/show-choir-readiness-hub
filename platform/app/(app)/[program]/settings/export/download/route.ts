...
import { loadTripDoc, loadPacketData, loadBoardSnapshot } from "@/lib/pdf/queries";
...
// Export-all download (§13.2, T029). Director/admin. Streams a single zip of
// CSVs (roster/guardians/ledger/…) + every generated PDF (published packets, bus
// + room sheets per trip, board snapshot per season). The caller's RLS client is
// the tenant boundary — every query runs as the signed-in director, so nothing
// crosses programs. Node runtime (React-PDF).
//
// Sync-build-then-download is fine at pilot scale (one program, a handful of
// competitions). Upgrade path when programs get large: enqueue an Inngest job
// that builds the zip, uploads it to Storage, and emails a signed link (§13.2) —
// the CSV/PDF builders here move into that job unchanged.
